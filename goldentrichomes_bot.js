...
app.post('/order', async (req, res) => {
  try {
    ...
    if (db && order.items?.length) {
      for (const item of order.items) {
        ...
        try {
          let snap = null;
          if (villeId) {
            const pSnap = await db.collection("boutiques").doc(villeId)
              .collection("produits").where("nom", "==", item.name).limit(1).get();
            if (!pSnap.empty) snap = pSnap;
          }
          if (!snap || snap.empty) {
            snap = await db.collection("products").where("name", "==", item.name).limit(1).get();
          }
          if (snap && !snap.empty) {
        ...
        } catch(se) { console.error("Stock check:", se.message); }
      }
    }
    }
    ...
    if (db) {
      ...
      /* -- Decremente le stock dans boutiques ET global -- */
      const vid = order.villeId || order.shop || null;
      for (const item of (order.items || [])) {
        if (!item.name) continue;
        try {
          const demande = (item.weight || item.w || 0) * (item.qty || 1);
          const updateStock = async (ref, prod) => {
            if (prod.stockGrams != null) {
              const nStock = Math.max(0, prod.stockGrams - demande);
              await ref.update({ stockGrams: nStock, stock: nStock===0?"out":nStock<10?"low":"available" });
            }
          };
          if (vid) {
            const bSnap = await db.collection("boutiques").doc(vid)
              .collection("produits").where("nom","==",item.name).limit(1).get();
            if (!bSnap.empty) await updateStock(bSnap.docs[0].ref, bSnap.docs[0].data());
          }
          const gSnap = await db.collection("products").where("name","==",item.name).limit(1).get();
          if (!gSnap.empty) await updateStock(gSnap.docs[0].ref, gSnap.docs[0].data());
        } catch(se) { console.error("Stock decrement:", se.message); }
      }
    }
